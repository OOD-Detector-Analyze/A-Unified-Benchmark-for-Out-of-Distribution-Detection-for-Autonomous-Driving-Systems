...
class UdacityImageTestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'attack' and idx % self.attack_frequency == 0:
            try:
                image = self.apply_attack(image)
                label = 1  # mark as adversarial
            except Exception as e:
                print(f"⚠️ Attack failed on idx {idx}: {e}")
                label = 0  # fallback to clean
        else:
            label = 0
